                                   ...
                                   uint32_t srcL2Id, uint32_t dstL2Id, bool isTx, Ptr<Packet> p)
{
  ...
  p->PeekHeader (pc5smt); //deserialise l'en-tete pour avoir le type de message de controle
  *stream->GetStream () << Simulator::Now ().GetSeconds (); //on stocke le temps
  if (isTx)
    {
      *stream->GetStream () << "\t" << "TX";
    }
...
{
  std::cout << "paquet envoye" <<std::endl;
  std::ofstream outfile("packets_envoyes.txt", std::ios::app);//open file in append mode
  if (outfile.is_open())
    {
      outfile << "paquet envoye depuis : " << addr << " avec taille " << pkt->GetSize() << " octets" << std::endl;
      outfile.close();
    }
  else
    {
      std::cerr << "Failed to open the file packets_envoyes.txt for writing" << std::endl;
    }
}

//TraceCallback function to capture received packets
void SinkRxTrace (std:: string context, Ptr<const Packet> pkt, const Address &addr)
{
std::cout << "packet received at: " << Simulator::Now().GetSeconds()<< std::endl;
}
//gNB's location
...

{
  ...
  std::string simTag = "montest"; //flux en voie montante de relais et remote
  double simTime = 15; // seconds

  //Setup large enough buffer size to avoid overflow
  Config::SetDefault ("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue (999999999));

  //Enable metaData for animation xml file
...
/**************************************************SL IEs configuration*********************************************/
 //SlResourcePoolNr IE
  ...
  nrHelper->AttachToClosestEnb (relayUeNetDev, enbNetDev);
   ...
   //Configure out-of-network UEs (Remote UE in this case)
  ...
  for (uint32_t u = 0; u < remoteUeNode.GetN (); ++u)
    {
     ...
     /*********************************************Récupération des traces; client /server********************************/
      //Install TracedCallback on the dlClient
      //Config::Connect("/NodeList/[i]/ApplicationList/[i]/$ns3::UdpClient",
                //MakeCallback(&SentPacketTrace));
      //dlApp.Get(0)->TraceConnectWithoutContext("TX", MakeCallback(&SentPacketTrace));
      
  /**************************************************************************************************/
    }
  //Configuration des traces sur server et client 
  // Pour le client remote host)
  for (uint32_t zx = 0; zx < clientApps.GetN (); ++zx)
  {
    Ptr<UdpClient> myclient = DynamicCast <UdpClient> (clientApps.Get(zx));
    std::cout << &myclient << std::endl;
    NS_ASSERT(myclient != nullptr);
    std::stringstream ss;
    ss << zx;
    myclient->TraceConnect("TX",ss.str(), MakeCallback(&SentPacketTrace));

  }
   // Pour le server (remote UE)
  for (uint32_t zz = 0; zz < serverApps.GetN (); ++zz)
  {
    Ptr<PacketSink> myserver = DynamicCast <PacketSink> (serverApps.Get(zz));
    NS_ASSERT(myserver != nullptr);
    std::stringstream ss;
    ss << zz;
    myserver->TraceConnect("RX",ss.str(), MakeCallback(&SinkRxTrace));
  }

  //Start and stop time
  serverApps.Start (Seconds (StartTime));
  serverApps.Stop (Seconds (simTime + cleanup_time)); //+cleanup_time pour laisser le temps de récuperer le dernier paquet
  ...
  Ptr<OutputStreamWrapper> Pc5SignallingPacketTraceStream = ascii.CreateFileStream (pc5s_filename.c_str ()); //creation de flux de sortie
  *Pc5SignallingPacketTraceStream->GetStream () << "time(s)\tTX/RX\tsrcL2Id\tdstL2Id\tmsgType" << std::endl; //ligne d'en tete dans le fichier de trace nouvellement cree
  ...
  *RelayNasRxPacketTraceStream->GetStream () << "time(s)\tnodeIp\tsrcIp\tdstIp\tsrcLink\tdstLink" << std::endl;
  ...
  //Netanim file
  AnimationInterface anim("monfichier-animation.xml");
  anim.EnablePacketMetadata(true);

  //Generate XML file
  monitor->SerializeToXmlFile("mon-fichier-test-XML",true,true);
