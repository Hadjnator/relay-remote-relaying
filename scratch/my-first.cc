  ...
  for (uint32_t u = 0; u < remoteUeNode.GetN (); ++u)
    {
      ...
      /***************Modification pour rattacher les traces*******************/
      Ptr<PacketSink> myclient = DynamicCast<PacketSink> (serverApps.Get (u));
      NS_ASSERT (myclient != nullptr);
      std::stringstream ss;
      ss << u;
      myclient->TraceConnect ("Rx", ss.str (), MakeCallback (&PrintRxPkt));
      /*****************************FIN modification***************************/
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if (i->second.rxPackets > 0)
        {
          outFile << "    Throughpout: " << i->second.rxBytes * 8.0 / appDuration / 1000 / 1000  << " Mbps\n";
